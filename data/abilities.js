...
let BattleAbilities = {
  ...
  pychomoxie: {
    ...
    shortDesc:
      "This Pokemon's Special attack is raised by 1 stage if it attacks and KOes another Pokemon.",
  ...
  dazzling: {
    ...
    onFoeTryMove: function(target, source, effect) {
      if (
        (source.side === this.effectData.target.side ||
          effect.id === "perishsong") &&
        effect.priority > 0.1 &&
  ...
  deltastream: {
    ...
    onAnySetWeather: function(target, source, weather) {
      if (
        this.getWeather().id === "deltastream" &&
        !["desolateland", "primordialsea", "deltastream"].includes(weather.id)
      )
        return false;
    },
  ...
  desolateland: {
    ...
    onAnySetWeather: function(target, source, weather) {
      if (
        this.getWeather().id === "desolateland" &&
        !["desolateland", "primordialsea", "deltastream"].includes(weather.id)
      )
        return false;
    },
  ...
  disguise: {
    ...
    onDamage: function(damage, target, source, effect) {
      if (
        effect &&
        effect.effectType === "Move" &&
        ["mimikyu", "mimikyutotem"].includes(target.template.speciesid) &&
        !target.transformed
      ) {
    ...
    onEffectiveness: function(typeMod, target, type, move) {
      ...
      if (
        !["mimikyu", "mimikyutotem"].includes(pokemon.template.speciesid) ||
        pokemon.transformed ||
    ...
    onUpdate: function(pokemon) {
      if (
        ...
        let templateid =
          pokemon.template.speciesid === "mimikyutotem"
    ...
    onStart: function(source) {
      for (const action of this.queue) {
        if (
          ...
          source.template.speciesid === "kyogre"
        )
          return;
    ...
    onStart: function(source) {
      for (const action of this.queue) {
        if (
          action.choice === "runPrimal" &&
          action.pokemon === source &&
          source.template.speciesid === "groudon"
        )
          return;
        if (action.choice !== "runSwitch" && action.choice !== "runPrimal")
          break;
  ...
  flowergift: {
    ...
    onUpdate: function(pokemon) {
      if (
        !pokemon.isActive ||
        pokemon.baseTemplate.baseSpecies !== "Cherrim" ||
        pokemon.transformed
      )
        return;
      if (this.isWeather(["sunnyday", "desolateland"])) {
  ...
  forecast: {
    ...
    onUpdate: function(pokemon) {
      if (
        pokemon.baseTemplate.baseSpecies !== "Castform" ||
        pokemon.transformed
      )
        return;
      let forme = null;
  ...
  harvest: {
    ...
    onResidual: function(pokemon) {
      if (
        this.isWeather(["sunnyday", "desolateland"]) ||
        this.randomChance(1, 2)
      ) {
        if (
          pokemon.hp &&
          !pokemon.item &&
          this.getItem(pokemon.lastItem).isBerry
        ) {
    ...
    onResidual: function(pokemon) {
      ...
      for (const allyActive of pokemon.side.active) {
        if (
          allyActive &&
          allyActive.hp &&
          this.isAdjacent(pokemon, allyActive) &&
          allyActive.status &&
  ...
  magnetpull: {
    ...
    onFoeMaybeTrapPokemon: function(pokemon, source) {
      if (!source) source = this.effectData.target;
      if (
        (!pokemon.knownType || pokemon.hasType("Steel")) &&
        this.isAdjacent(pokemon, source)
      ) {
        pokemon.maybeTrapped = true;
      }
    },
  ...
  minus: {
    ...
    onModifySpA: function(spa, pokemon) {
      ...
      for (const allyActive of pokemon.side.active) {
        if (
          allyActive &&
          allyActive.position !== pokemon.position &&
          !allyActive.fainted &&
          allyActive.hasAbility(["minus", "plus"])
        ) {
          return this.chainModify(1.5);
        }
      }
  ...
  mummy: {
    desc:
      "Pokemon making contact with this Pokemon have their Ability changed to Mummy. Does not affect the Battle Bond, Comatose, Disguise, Multitype, Power Construct, RKS System, Schooling, Shields Down, and Stance Change Abilities.",
    shortDesc:
  ...
  noguard: {
    ...
    onAnyAccuracy: function(accuracy, target, source, move) {
      if (
        move &&
        (source === this.effectData.target || target === this.effectData.target)
      ) {
        return true;
      }
      return accuracy;
  ...
  overcoat: {
    ...
    onTryHit: function(target, source, move) {
      if (
        move.flags["powder"] &&
        target !== source &&
        this.getImmunity("powder", target)
      ) {
  ...
  pickup: {
    ...
    onResidual: function(pokemon) {
      ...
      let pickupTargets = [];
      for (const target of pokemon.side.active.concat(
        pokemon.side.foe.active
      )) {
        if (
          target.lastItem &&
          target.usedItemThisTurn &&
          this.isAdjacent(pokemon, target)
        ) {
  ...
  powerconstruct: {
    ...
    onResidual: function(pokemon) {
      if (
        pokemon.baseTemplate.baseSpecies !== "Zygarde" ||
        pokemon.transformed ||
        !pokemon.hp
      )
        return;
      if (
        pokemon.template.speciesid === "zygardecomplete" ||
      ...
      let newHP = Math.floor(
        (Math.floor(
          2 * pokemon.template.baseStats["hp"] +
            pokemon.set.ivs["hp"] +
  ...
  primordialsea: {
    ...
    onAnySetWeather: function(target, source, weather) {
      if (
        this.getWeather().id === "primordialsea" &&
        !["desolateland", "primordialsea", "deltastream"].includes(weather.id)
      )
        return false;
    },
  ...
  schooling: {
    ...
    onStart: function(pokemon) {
      if (
        pokemon.baseTemplate.baseSpecies !== "Wishiwashi" ||
        pokemon.level < 20 ||
        pokemon.transformed
      )
        return;
  ...
  shieldsdown: {
    ...
    onResidual: function(pokemon) {
      if (
        pokemon.baseTemplate.baseSpecies !== "Minior" ||
        pokemon.transformed ||
        !pokemon.hp
      )
        return;
  ...
  stickyhold: {
    ...
    onTakeItem: function(item, pokemon, source) {
      if (
        (this.suppressingAttackEvents() && pokemon !== this.activePokemon) ||
        !pokemon.hp ||
  ...
  trace: {
    ...
    onUpdate: function(pokemon) {
      ...
      let possibleTargets = pokemon.side.foe.active.filter(
        foeActive => foeActive && this.isAdjacent(pokemon, foeActive)
      );
  ...
  truant: {
    ...
    onStart: function(pokemon) {
      ...
      if (
        pokemon.activeTurns &&
        (pokemon.moveThisTurnResult !== undefined || !this.willMove(pokemon))
      ) {
  ...
  zenmode: {
    ...
    onResidual: function(pokemon) {
      if (
        pokemon.baseTemplate.baseSpecies !== "Darmanitan" ||
        pokemon.transformed
      ) {
        return;
      }
      if (
        pokemon.hp <= pokemon.maxhp / 2 &&
        pokemon.template.speciesid === "darmanitan"
      ) {
        pokemon.addVolatile("zenmode");
      } else if (
        pokemon.hp > pokemon.maxhp / 2 &&
        pokemon.template.speciesid === "darmanitanzen"
      ) {
